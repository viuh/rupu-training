...
def train(X, y):
    ...
    X_train = np.array(
        [imageio.imread(uri, as_gray=False, pilmode="RGB") for uri in X_train["uri"]])
    X_test = np.array([imageio.imread(uri, as_gray=False, pilmode="RGB")
                       for uri in X_test["uri"]])
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            cucumbers = pd.read_csv("./data/cucumber.csv")[1:30]
            mopeds = pd.read_csv('./data/moped.csv')[1:30]
        except:
        ...
        merged = pd.concat(
            [pd.read_csv("./data/cucumbers2.csv"), pd.read_csv("./data/mopeds2.csv")])
